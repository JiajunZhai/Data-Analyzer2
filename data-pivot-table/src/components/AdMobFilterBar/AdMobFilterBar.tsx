  ...
  return (
    <div className="filter-chip-wrapper" ref={dropdownRef}>
      ...
      >
        ...
        {dimensionHint && <span className="chip-dimension-hint" title={dimensionHint}><BarChart3 size={11} /></span>}
        ...
        <div className="filter-dropdown-menu" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="filter-dropdown-options">
            ...
            {filteredValues.length === 0 && (
              <div className="filter-dropdown-empty">无匹配项</div>
            )}
          </div>
      ...
      {configs.map(config => (
        config.type === 'date' ? (
          ...
          <FilterChip
            ...
            selectedValues={getSelectedValues(config.fieldName)}
            onSelectionChange={(values) => onFilterChange(config.fieldName, values)}
            dimensionHint={activeDimensionNames?.has(config.fieldName) ? `已作为维度使用` : undefined}
          />
        )
      ))}
    </div>
