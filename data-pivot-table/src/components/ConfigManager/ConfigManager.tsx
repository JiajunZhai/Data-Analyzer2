...
import { Settings, Save, Pencil, Trash2, Smartphone, Globe, Megaphone, Film } from 'lucide-react';
import type { Field, PivotField, FilterConfig } from '../../types';
...
const ConfigManager: React.FC<ConfigManagerProps> = ({
  ...
  const hasCurrentConfig = currentConfig.rowFields.length > 0 || currentConfig.colFields.length > 0 || currentConfig.valueFields.length > 0;

  const getConfigSummary = (config: { rowFields: PivotField[]; colFields: PivotField[]; valueFields: PivotField[] }) => {
    const parts: string[] = [];
  ...
  const handleTemplateSelect = useCallback((template: PivotTemplate) => {
  ...
  return (
    <div className="config-manager-wrapper" ref={dropdownRef}>
      ...
      >
        <span className="capsule-icon"><Settings size={14} /></span>
        ...
        <div className="config-dropdown" onClick={(e) => e.stopPropagation()}>
          ...
          {currentDatasetId && (
            <div className="config-save-section">
              <button className="btn-quick-save" onClick={handleQuickSave} disabled={!hasCurrentConfig}>
                  ...
                  {editingId === config.id ? (
                    ...
                    <>
                      ...
                      <div className="config-actions">
                        <button onClick={() => handleRenameStart(config)} title="重命名"><Pencil size={14} /></button>
                        <button onClick={(e) => handleDelete(config.id, e)} title="删除"><Trash2 size={14} /></button>
                      </div>
                    </>
                  )}
                </div>
